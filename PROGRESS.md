...
> - **⚠️ 배포 방법 = 커밋·푸시하면 자동**: `main` push → GitHub Actions가 서버 SSH→`deploy.sh`(git pull + `compose up -d --build` + caddy restart). mes-frontend 이미지가 서버에서 빌드되고 Caddy 라우팅 반영됨. **미커밋 신규**: `hwlee-mes/frontend/{Dockerfile,Caddyfile,.dockerignore}`. **미커밋 수정**: `docker-compose.prod.yml`·`caddy/Caddyfile`·`hwlee-mes/build.gradle.kts`·`PROGRESS.md`. **미커밋 삭제**: `hwlee-mes/.../web/MesViewController.java`·`hwlee-mes/src/main/resources/templates/`. (어제 Vue 프로젝트·시뮬레이터는 커밋 `e181d7b`로 이미 반영됨.)
>
...
> - **▶ 다음 후보**: ⓐ 브라우저 육안 확인(http://localhost:5174) ⓑ 다른 모듈 확장(출하 delivery/청구 invoice) ⓒ 정적 빌드 Caddy 배포(erp.hyunwoo.pro, MES 배포 패턴 재사용) ⓓ React 코드 워크스루(학습).
