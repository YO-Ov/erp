...
>
> - **✅ MES 정적 배포 구성 완료(2026-07-16) — 후보①**: `mes.hyunwoo.pro`에서 Vue SPA(정적)를 서빙하도록 인프라 구성. **설계 = 프론트를 독립 정적 컨테이너로 추가 + 메인 Caddy에서 경로 분기**(방식 B). 기존엔 `mes.hyunwoo.pro`가 Caddy→`mes:8082`(Thymeleaf) 통짜 프록시였는데, 이제 **`/api/*`·`/actuator/*`→`mes:8082`(백엔드 무수정), 그 외 전부→`mes-frontend:80`(Vue dist)**. 신규 파일: `hwlee-mes/frontend/Dockerfile`(멀티스테이지 node22 빌드→`caddy:2-alpine` 정적 서버) · `frontend/Caddyfile`(:80, `try_files→/index.html` SPA 폴백, `auto_https off`) · `frontend/.dockerignore`. 수정: `docker-compose.prod.yml`(`mes-frontend` 서비스 추가, 외부포트 없음·`depends_on: mes`, caddy `depends_on`에 추가) · `caddy/Caddyfile`(mes 블록 경로 분기). API는 `client.js`가 `baseURL:'/api'` 고정이라 같은 도메인으로 나감(무수정). ⚠️ **이 변경으로 기존 Thymeleaf MES 화면은 대체됨**(루트=Vue SPA, 통째 전환 목표대로). **✅ 로컬 e2e 검증**: `docker build` 그린(99모듈) → 컨테이너 기동 → 루트 200(Vue `#app`+모듈스크립트)·**SPA 딥링크 `/work-orders/9`→200**·JS asset 200(text/javascript) 확인. `docker compose config` OK(mes-frontend 인식)·메인 Caddyfile `adapt` 파싱 정상.
> - **✅ Thymeleaf 완전 제거(2026-07-16)**: Vue가 기존 Thymeleaf 화면(작업지시 목록·상세 2개)을 전부 대체+초과(접수폼·설비대시보드·품질검사·시뮬레이터 추가) 확인 → dead code 제거. 삭제: `MesViewController.java`(+빈 `web/` 패키지) · `templates/`(layout·workorder/list·detail) · `build.gradle.kts` thymeleaf 의존성. templates 참조 코드 없음(grep 확인). **✅ `./gradlew compileJava --rerun-tasks` 그린**(제거 후 정상 빌드). → **MES는 화면 관점에서 Vue 전환 완료**(백엔드 REST는 무수정 유지).
> - **✅ 배포·운영 검증 완료(2026-07-16)**: 커밋 `8aeb8c4` → main push → GitHub Actions 자동배포(약 3분15초). **✅ 운영 e2e**: `https://mes.hyunwoo.pro/` = 200 Vue index.html(이전 302 리다이렉트→해소)·딥링크 `/work-orders`·`/equipments` = 200 SPA 폴백(이전 Thymeleaf 3538B→SPA 403B)·JS asset 200(text/javascript)·`/api/work-orders` = 200(경로 분기 정상). **경로 분기(`/api`→mes:8082, 그 외→mes-frontend)가 운영에서 정상 동작 확인.** ⚠️ 이 서버 mes_db엔 작업지시 데이터 없음(`/api/work-orders`=`[]`) → 화면은 정상이나 목록 비어보임. 데모하려면 Vue 접수폼(`/work-orders/new`)으로 작업지시 생성 필요(로컬 데모데이터 id=9,10은 로컬 docker 전용). **⚠️ push에 쓴 GitHub PAT가 채팅에 노출됨 → 폐기·재발급 권장.**
> - **🗄️ 로컬 docker mes_db → AWS RDS 이관(2026-07-16)**: 어제 작업분이 로컬 docker(:3308)에만 쌓여 있어 AWS RDS(`application-aws.yml`, `mes_db`/`mes_app`)로 옮김. 스키마는 양쪽 Flyway V1~V9 동일 → **데이터만 교체**(FK 자식→부모 순 DELETE + `mysqldump --no-create-info` 적재, 단일 트랜잭션). 이관 10개 테이블 = work_order 10·work_order_line 21·production_result 29·quality_inspection 1·material_consumption 42·outbox_event 30(전부 SENT라 Kafka 재발행 없음)·equipment_status_log 14·equipment 8·operator 8·defect_reason 3. **✅ 검증**: 테이블별 `mysqldump | md5` 대조 10/10 일치. ⚠️ **이관 중엔 백엔드를 내려야 함** — `aws` 프로필로 뜬 시뮬레이터가 IN_PROGRESS 지시(id=10)를 물고 데이터를 덮어씀.
> - **📱 MES Vue 반응형 대응 + 헤더 탭 정리(2026-07-17, hwlee님 관찰)**: "헤더 탭이 붙어 있고 선택 표시가 안 보인다, 하다 만 느낌" 지적에서 출발. **ⓐ 헤더 탭**: `.nav`에 flex/gap이 없어 Vue 템플릿 공백압축으로 두 링크가 `작업지시설비 가동현황`으로 붙었고, 활성 표시가 `color`(muted→text)뿐이라 안 보였음 → pill 탭(패딩·hover·focus-visible 링, 활성=accent 옅은 배경. 채움버튼 `.primary`와 시선 경쟁 피함). **🐞 같이 잡은 버그**: 상세/접수에서 "작업지시" 탭 활성이 풀림 — vue-router 4는 경로 prefix가 아니라 **라우트 record 계층**으로 판정하는데 `/work-orders`와 `/work-orders/:id`가 독립 최상위 라우트라 부모·자식이 아니었음 → `inSection()` 경로 접두사 판정 + `.tab-active` 로 교체. **ⓑ 반응형(640px 브레이크포인트)**: 헤더(brand-sub 접기)·page-head(wrap+버튼 균등)·작업지시 목록(6컬럼 표→**행 카드 전환**)·설비(상태버튼 4→2×2, `minmax(min(280px,100%),1fr)`)·상세(info 라벨 위/값 아래, 실적폼 재배치)·접수(BOM 입력표→**라인별 입력 블록**)·소요자재/품질이력 표는 `.table-scroll`(페이지 대신 표만 스크롤). **🐞 CSS 우선순위 버그**: 전역 유틸 `.only-mobile{display:none}`(0-1-0)보다 scoped `.wo-cards[data-v]{display:flex}`(0-2-0)가 이겨 **데스크톱에서 표+카드 동시 렌더** → 각 컴포넌트가 자기 미디어쿼리로 켜도록 수정(`.only-desktop`만 전역 유틸로 남김). **✅ 검증**: 320/375/768/1280px에서 `documentElement.scrollWidth == innerWidth` 실측 일치(가로 넘침 0), `npm run build` 그린. ⚠️ **검증 함정**: `chrome --window-size=375`는 macOS 창 최소폭 때문에 실제 뷰포트가 **500px**로 잡혀 스크린샷이 왼쪽만 잘려 나옴(멀쩡한 레이아웃이 깨져 보임) → **CDP `Emulation.setDeviceMetricsOverride`** 로 진짜 모바일 뷰포트를 만들어야 함(Node 내장 WebSocket으로 가능, 패키지 설치 불필요).
> - **🔷 MES Vue → TypeScript 전환 완료(2026-07-17, hwlee님 요청 "실무에서 Vue만 쓰나?" → 신규 Vue 3 프로젝트는 TS가 사실상 표준이라 확인 후 착수)**: 프론트 14개 파일(약 2,000줄) 전면 TS 전환. **범위 = 언어 전환만, 기능·UI·백엔드 무수정**. ⓐ **설치**: `typescript`·`vue-tsc`·`@types/node`. **⚠️ 함정 — TypeScript 7(Go 재작성 tsgo)은 vue-tsc 와 아직 비호환**(`ERR_PACKAGE_PATH_NOT_EXPORTED: './lib/tsc'` — TS7이 vue-tsc가 쓰는 내부 경로 export 를 없앰). peer 는 `>=5.0.0` 이라 설치는 되지만 실행이 죽음 → **`typescript@~5.9` 로 고정**(ERP 전환 때도 동일 주의). ⓑ **설정**: `tsconfig.json`(references) + `tsconfig.app.json`(strict·noUnusedLocals·bundler 해석, `noEmit` — 트랜스파일은 Vite 담당) + `tsconfig.node.json`(vite.config 용). ⓒ **`src/types/api.ts` 신설 = 백엔드 계약의 단일 출처** — Java record 와 1:1(BigDecimal→number, LocalDate/Time→string, enum→문자열 리터럴 유니온). ⓓ `.js`→`.ts` 6개(client·workOrders·status·store·router·main + vite.config), `.vue` 7개 `<script setup lang="ts">`. `StatusBadge` 는 타입기반 `defineProps<{label:string; tone?:Tone}>()` — tone 오타가 컴파일에서 걸림. ⓔ `errorMessage(e: unknown)` 을 `api/client.ts` 에 추가(catch 파라미터가 unknown 이라 6곳 반복 제거). ⓕ `package.json` 에 **`type-check`(vue-tsc --build)** 추가 + **`build` = 타입검사 → vite build**(Dockerfile 이 `npm run build` 를 타므로 **타입 에러 시 배포가 막힌다**). `.gitignore`·`.dockerignore` 에 `*.tsbuildinfo`.
>   - **🐞 TS 전환이 잡아낸 실제 버그 — 실적 등록이 화면 상태를 깨뜨림**: `POST /work-orders/{id}/results` 는 **`ProductionResultResponse`(등록된 실적)** 를 돌려주는데(다른 액션 start·pause·resume·complete 는 전부 `WorkOrderResponse`), 상세화면이 이걸 `run()` 공통 래퍼에 통과시켜 **`wo.value = 실적객체` + `store.replaceOne(실적객체)`** 를 하고 있었음. 실적 객체엔 `workOrderNo`·`quantity`·`status`·`lines` 가 없어 **화면의 작업지시 정보가 통째로 깨짐**(IN_PROGRESS 면 2.5초 폴링이 곧 덮어써서 눈에 잘 안 띄었을 뿐, PAUSED 등에선 그대로 남음). `replaceOne` 은 **실적 id** 로 목록을 뒤져서, 실적 id 가 작업지시 id 와 겹치면 **목록의 엉뚱한 행까지 실적 객체로 교체**될 수 있었음. **수정**: `doReport` 가 실적 등록 후 `findById` 로 갱신된 작업지시를 다시 읽어 반환(`run` 시그니처를 `() => Promise<WorkOrder>` 로 못박아 재발 방지). API 계층에도 반환 타입 차이를 주석으로 명시. ⓑ 부수로 잡힌 것: `WorkOrderDetailView` 의 미사용 `useRouter`(noUnusedLocals), 설비 대시보드의 `util(code).toFixed()` **null 역참조 가능**(v-if 로 검사해도 함수 호출은 narrowing 안 됨) → `utilText()` 로 분리.
>   - **✅ e2e 검증(실 백엔드 :8082 + CDP 실브라우저)**: `npm run type-check` 그린 / `npm run build` 그린(106모듈, gzip 39KB) / **`docker build` 그린**(컨테이너 안에서 타입검사까지 통과 = 배포 경로 안전) / **실적 등록 실동작** — id=10 상세에서 양품 7 등록 → 작업지시 정보 유지·생산 0→7·폼 초기화·런타임 예외 0(수정 전이라면 여기서 화면이 깨졌음) / 목록·설비·접수폼 4화면 렌더링 정상, 런타임 예외 0, 1280·375px 가로 넘침 0 / 설비 가동률 `0.0%`·`집계 전` 분기 정상. ⚠️ **검증 부작용**: `WO-LIVE-DEMO-002`(id=10)에 **실적 7개가 실제로 등록됨**(생산 0→7, 자재 7 투입). 데모용이라 무방하나 "0부터 차오르는" 시연을 원하면 새 작업지시를 만들 것.
>   - **▶ 다음**: hwlee님 계획대로 **ERP(React)도 TS 전환** — MES 에서 정립한 패턴 그대로(types/api.ts 로 백엔드 계약 고정 → api/ → domain → 화면). ⚠️ **ERP 는 재무(FI) 분량이 미커밋 상태라 먼저 커밋**해야 "기능 추가"와 "TS 전환"이 diff 에서 안 섞임.
>
> #### ✅ ERP React SPA 착수 1차 완료 — 견적·수주 모듈 (2026-07-16)
> 확정 순서(MES Vue → ERP React)대로 **ERP를 React로 착수**. 신규 `hwlee-erp/frontend/` = **Vite + React 19 + React Router 7 + TanStack Query 5 + Axios**. 백엔드 무수정(Vite 프록시 `:5174 → :8080`, MES 5173과 포트 분리).
> - **인증 = JWT Bearer 헤더 방식**(hwlee님 선택, 백엔드는 Bearer·ACCESS_TOKEN 쿠키 둘 다 지원하나 SPA 표준인 헤더 채택 — MES엔 없던 계층). `auth/auth.js`(localStorage 토큰 저장) + `api/client.js`(요청 인터셉터가 `Authorization: Bearer` 자동첨부, 응답 401 → 토큰 클리어 + `erp:unauthorized` 이벤트, ProblemDetail→한글 메시지 정규화) + `auth/AuthContext.jsx`(login `POST /api/auth/login`·logout·hasRole) + `auth/ProtectedRoute.jsx`(미로그인 시 /login 리다이렉트, 원경로 보존) + `LoginView`. refresh 토큰 없음(1시간 만료 시 재로그인).
> - **첫 모듈 = 견적+수주**(hwlee님 선택). 화면 6종: 견적 목록(상태·발행일 검색·페이징)·상세(라인·상태배지·전이액션 상신/발송/수락/취소, 수락→수주생성 이동)·작성(고객·품목 드롭다운+동적 라인). 수주 목록·상세(견적 역링크·출하/청구량)·등록(**견적→수주 전환** `?quotationId=` 프리필+여신현황 표시). `domain/status.js`에 견적(DRAFT→APPROVED→SENT→ACCEPTED→CONVERTED…)·수주(DRAFT→CONFIRMED→…→CLOSED) enum 한글라벨·상태별 액션 규칙. 조회는 TanStack Query `useQuery`, 폼/액션은 `useMutation`+캐시 무효화.
> - **✅ e2e 검증**: `npm run build` 그린(156모듈, gzip 111KB). 백엔드 실 API(admin 로그인 200/roles=ADMIN, 견적 4건·수주 781건·고객 100·품목 50) 응답이 프론트 계약(Page 구조·필드명)과 정확히 일치. **Vite 프록시(5174→8080) 경유** SPA index 200 + 로그인 200 확인. 데모 계정 `admin@hyunwoo.com`/`pass1234`.
> - ⚠️ `frontend/`는 erp git에 잡히나 `node_modules`·`dist`는 자체 `.gitignore` 제외. **미커밋** — 다른 PC 이어가려면 커밋·푸시 필요.
> - **✅ 로그인 시연 계정 select + footer + 역할 접근제어 추가(2026-07-16, hwlee님 요청)**: ⓐ 로그인 화면에 기존 Thymeleaf와 동일한 **"빠른 선택(시연용)" 드롭다운**(영업/재무/구매/생산/인사/관리자 16계정 optgroup, 고르면 이메일+공통비번 pass1234 자동입력) ⓑ 앱 전체 **footer**(`© 2026 HYUNWOO ERP …`, sticky) ⓒ **역할 접근제어**: 견적·수주는 백엔드가 SALES/ADMIN만 허용 → `ProtectedRoute`에 `roles` 옵션 추가(권한 없으면 `AccessDenied` 안내 화면), Header 네비도 `hasRole`로 SALES/ADMIN에만 노출. **✅ 검증**: lee(FINANCE)→견적 API 403(프론트가 미리 차단) / kim(SALES)→200, npm build 그린.
> - **✅ 견적·수주 수정(PUT) 화면 추가(2026-07-16)**: 작성 화면(`QuotationCreateView`/`SalesOrderCreateView`)을 **작성/수정 겸용**으로 확장(`/quotations/:id/edit`·`/sales-orders/:id/edit` 라우트, SD 권한). 기존 건 로드→폼 프리필, update DTO 제약대로 **고객 고정**(견적: customerId 미전송 / 수주: customerId·quotationId 미전송, 영업담당 salespersonId는 기존값 보존). 상세화면에 **DRAFT 상태에서만 '수정' 버튼** 노출(상신·확정 후엔 이력 정합상 수정 불가 방침). **✅ e2e**: DRAFT 견적 생성(total 2000)→PUT 수정→total 5000·유효기한·수량 반영 확인, npm build 그린.
> - **✅ 출하·청구 모듈 추가 = SD 흐름 완성(2026-07-16)**: 견적→수주→**출하(delivery)→청구(invoice)** 4단계 SD 파이프라인 완성. 출하·청구 각 목록(상태·날짜 검색·페이징)·상세(수주 역링크·라인·취소)·**수주 라인 기반 생성**(수주 상세에서 "출하 생성"/"청구 생성" 진입 `?salesOrderId=`, 각 라인 잔여=수주량−기출하/기청구량 기본값). 출하=창고 선택+출하일, 청구=청구일(+세액은 서버 계산, 상세에 공급가/세액/합계 표시). 상태: 출하 DRAFT/SHIPPED/CANCELLED·청구 DRAFT/ISSUED/CANCELLED(`domain/status.js`, `canCreateDownstream`=CONFIRMED~INVOICED). 권한 SD_ROLES(SALES/ADMIN) 동일. 네비 메뉴 출하·청구 추가. ⚠️ DeliveryResponse에 warehouseName 없어(백엔드 무수정) 프론트가 창고목록으로 id→이름 매핑. **✅ e2e**: 수주 생성→confirm→출하(SHIPPED,5개)→청구(ISSUED, 공급가25000+세액2500=27500)→수주 재조회 shippedQty/invoicedQty 반영·상태 INVOICING 확인. npm build 그린(114KB).
> - **✅ 구매(MM) 발주 모듈 추가 = 2번째 도메인 전환 착수(2026-07-16)**: SD(영업) 다음으로 **구매**를 React로. 발주(purchase-order) 목록(공급처·상태·날짜 검색)·상세(공급처·창고·라인 발주량/입고량/미입고·비고)·작성/수정 겸용. 상태 DRAFT→CONFIRMED→RECEIVED→CLOSED(+CANCELLED), **confirm 엔드포인트 없음→DRAFT는 결재 상신(submit-approval)으로만 진행**, RECEIVED는 입고 시 자동전이. 액션 DRAFT=[상신·수정·취소]/CONFIRMED=[종료·취소]/RECEIVED=[종료]. **⚠️ SD와 다른 2가지**: ① **권한 분리** — 조회는 6역할(SALES/PURCHASING/PRODUCTION/FINANCE/DIRECTOR/ADMIN=`MM_VIEW_ROLES`), **쓰기는 PURCHASING/ADMIN만**(`MM_WRITE_ROLES`) → 라우트/네비/버튼 각각 반영(조회역할은 목록·상세 보되 작성/수정/액션 버튼 안 보임). ② **취급품목(구매정보레코드) 제약** — 발주 품목은 그 공급처의 `vendor-items`만 가능 → 폼에서 공급처 선택 시 품목 드롭다운을 취급품목으로 제한 + 품목 선택 시 공급단가(supplyPrice) 자동. **✅ e2e**: 권한(park 생산=조회200/쓰기403, purchase=쓰기OK) + 발주 생성(동부부품·GPU모듈×10=280만, DRAFT)→상신 200. 취급품목 아닌 품목은 409(거래처 취급품목 아님) 확인. npm build 그린(116KB).
> - **✅ 구매(MM) 입고 모듈 추가 = 구매 흐름 완성(2026-07-16)**: 발주→**입고(goods-receipt)**로 MM 파이프라인 완성. 입고 목록(공급처·상태·날짜 검색)·상세(발주 역링크·라인·전기일시)·**발주 기반 생성**(발주 상세 CONFIRMED/RECEIVED에서 "입고 처리" 진입 `?purchaseOrderId=`, 공급처·창고는 발주에서 고정, 라인은 미입고량(openQuantity) 기본·입고단가=발주단가 기본). 상태 DRAFT→POSTED(전기: 재고 가중평균 갱신+StockMovement+발주 RECEIVED 자동전이)→CANCELLED. 액션 DRAFT=[전기·취소]. **권한: 입고는 조회·쓰기 모두 PURCHASING/ADMIN 전용**(발주와 달리 조회도 제한, `MM_WRITE_ROLES`). 네비 "입고"도 구매/관리자에만. 발주 상세에 `canReceive`(CONFIRMED/RECEIVED)일 때 "입고 처리" 버튼. **✅ e2e**: 입고 생성(DRAFT)→전기(POSTED)→stock-movement에 GOODS_RECEIPT 이동 생성 확인. npm build 그린(117KB). ⚠️ **미검증**: 발주 연동 시 발주 RECEIVED 전이는 시드에 CONFIRMED 발주 없음(결재 승인 필요)으로 실 e2e 못 태움 — 백엔드 로직(GoodsReceiptService.post→purchaseOrderService.receive) 존재만 코드 확인. 발주 없는 입고로 create/post/재고반영은 검증됨.
> - **✅ 생산(PP) 작업지시 모듈 추가 = 3번째 도메인 전환(2026-07-16)**: 생산 작업지시(production-order) 목록(상태·지시일 검색)·상세·생성. 상태 PLANNED→RELEASED(착수)→COMPLETED(완료: 부품출고+완제품입고+생산분개)→CANCELLED. **생성은 완제품·수량·창고·일정만 입력** → 서버가 **BOM 전개**해 소요자재 라인 자동 생성(BOM 없으면 409). 상세에 **자재 가용성**(`material-availability`: 생산가능 배너 + 자재별 소요/재고/충분여부, PLANNED·RELEASED에서만 조회) + 소요자재 라인. 액션 PLANNED=[착수·취소]/RELEASED=[완료·취소]+**MES 전송(dispatch)**(RELEASED·미전송 조건). **⭐ ERP→MES 연동**: dispatch로 MES에 작업지시 전송→`mesWorkOrderNo` 수신·표시. 권한 PRODUCTION/ADMIN 전용(`PP_ROLES`). 네비 "생산" 추가. **✅ e2e**: 작업지시 생성(올인원PC 27"×5, BOM 7종 자동전개)→자재가용성 producible=true(재고 충분)→release(RELEASED)→**dispatch 성공(WO-20260716-001, mesStatus=RECEIVED)** 실동작. npm build 그린(118KB). complete(완료)는 재고소진 부작용으로 e2e 미실행, 코드 확인.
> - **▶ 다음 후보**: ⓐ 브라우저 육안 확인(http://localhost:5174) ⓑ 다음 도메인 = **재무(FI)** 또는 **인사(HR)** ⓒ 재고 조회 화면(현재고·이동이력) ⓓ BOM 조회 화면(생산 보조) ⓔ 정적 빌드 Caddy 배포 ⓕ React 코드 워크스루(학습).
> - **✅ 재무(FI) 모듈 추가 = 4번째 도메인 전환(2026-07-17)**: 전표·입출금·계정과목 7화면. 권한 **FINANCE/ADMIN 전용**(`FI_ROLES`, 조회/쓰기 구분 없음 — MM과 달리 단일 역할군). ⓐ **전표(journal-entries)** 목록(상태·출처·전표일 검색)·상세(분개 라인 차/대 + 균형 표시 + 연결 출처)·**수동 작성**. ⓑ **입출금(payments)** 목록(구분·거래일)·상세(**연결 전표 역링크** — `sourceType=PAY&sourceId=` 로 조회)·등록. ⓒ **계정과목(accounts)** 읽기전용 목록(유형 필터, 분류/말단 구분).
>   - **⭐ FI가 SD·MM·PP와 다른 4가지**(도메인 학습 포인트): ① **생성이 두 갈래** — `POST /journal-entries`(즉시 전기 POSTED) vs `POST /journal-entries/draft`(DRAFT→결재 상신→**최종 승인 시 자동 전기**). 폼에 '전기(확정)'·'초안 저장(결재용)' 버튼 2개로 반영. ② **차대 균형이 핵심 불변식** — 불일치 시 **422 `UNBALANCED_JOURNAL`**. 작성 폼이 차변합/대변합/차액을 실시간 표시하고 불균형이면 전기 버튼 비활성(초안 저장은 허용 — 균형 검증은 전기 시점). ③ **취소가 SD/MM과 반대** — **POSTED만 취소 가능, DRAFT는 취소 불가**(409). 회계는 확정 전표에 '취소된 사실'을 남기고 초안은 결재로만 진행. ④ **라인이 `accountId`가 아니라 `accountCode`** + `debit`/`credit`(한쪽만 >0). 계정 드롭다운은 **postable=true 말단 계정만**(분류 계정 분개 시 400).
>   - **입출금 정합성**: `RECEIPT`=고객 필수·vendorId 금지(차)현금/대)매출채권), `DISBURSEMENT`=공급처 필수·customerId 금지(차)매입채무/대)현금). 폼이 구분에 따라 한쪽 party만 전송·드롭다운 교체. **상신은 출금 초안만**(입금 상신 시 409) → 입금엔 초안 버튼 자체를 안 보임.
>   - **🐞 같이 고친 것 — 로그인 착지 버그**: 기본 라우트(`path="*"`)가 무조건 견적(SD)이라 **재무·구매·생산 역할은 로그인하자마자 '권한 없음' 화면**을 봤음 → `HomeRedirect`로 역할이 실제 볼 수 있는 첫 모듈로 착지. ⚠️ 순서 주의: `MM_VIEW_ROLES`가 재무·생산까지 포함해서 넓으므로 맨 뒤에 둬야 재무=전표, 생산=작업지시로 감.
>   - **✅ e2e 검증(전부 실 API·AWS RDS)**: 균형 전표 전기(JE-20260717-001, 차 법정복리비 5만/대 현금 5만 → POSTED) / **불균형 → 422 UNBALANCED_JOURNAL**(detail="차변 합 50000, 대변 합 30000") / 분류계정(1000 자산) 분개 → 400 / POSTED→**CANCELLED** / DRAFT 상신 → 결재요청 PENDING(docType=JOURNAL) / DRAFT 취소 → **409**("POSTED 전표만 취소 가능") / 입금 15만 전기 → **연결 전표 자동생성 확인**(차 1100 현금 / 대 1200 매출채권) / RECEIPT+vendorId → 400 / 출금 초안→상신 PENDING(docType=PAYMENT) / 입금 초안 상신 → 409 / **권한 SALES(kim) → 전표·입출금·계정과목 전부 403**. `npm run build` 그린(187모듈, gzip 122KB), Vite 프록시(5174→8080) 경유 SPA·딥링크·로그인 200.
>   - ⚠️ **RDS에 남은 e2e 데이터**(hwlee님 선택으로 RDS 대상 진행): 전표 `JE-20260717-001`(취소됨)·`-004`(DRAFT 상신중)·`-005`(입금 자동분개) / 입출금 `PAY-20260717-001`(입금 POSTED)·`-003`(출금 DRAFT 상신중)·입금초안 1건 / 결재요청 id=10,11 PENDING. 전부 적요에 `[e2e]` 표시. 지우려면 DB 직접 삭제 필요(취소 API로는 초안이 안 지워짐).
>   - ⚠️ **로컬 docker erp_db는 깨져 있음** — 예전 손데이터(`CUST-2026-0003 Apple` 등)가 시드 `V53__step4_customers.sql`(고객 100건 INSERT)과 코드 충돌 → **Flyway 실패로 `local` 프로필 기동 불가**. 되살리려면 `docker compose down -v` 로 볼륨 초기화 후 재기동(로컬 손데이터 4고객·7수주·5견적은 소실). 지금은 **`aws` 프로필로 RDS** 사용 중.
>
> - **✅ 전자결재 + 대시보드 추가 = 업무 흐름 닫힘(2026-07-17)**: 지금까지 각 모듈이 **상신만 하고 승인 화면이 없어** 결재요청이 PENDING으로 쌓이기만 하던 것을 해소. 3화면 신설. ⓐ **전자결재 목록**(`/approvals`) — 결재함(inbox, 내가 처리)/상신함(outbox, 내가 올림) 탭, 탭 상태는 `?box=outbox`로 URL에 보존(새로고침·뒤로가기 유지), `myTurn`이면 "● 내 차례" 강조. ⓑ **결재 상세**(`/approvals/:id`) — 문서정보 + 원본문서 링크 + **결재선 타임라인**(단계·유형(결재/합의/참조)·결재자·부서·상태·의견, 진행중 단계 ● 강조) + 액션(승인·반려·반송·회수·재상신) + 의견 입력. ⓒ **대시보드**(`/dashboard`) — 공통 위젯(결재대기·진행중상신·반려됨·미확인알림 카드 + 내 결재대기 + 내 상신현황 + 알림 목록·읽음/전체읽음) + **SALES면 영업 KPI 섹션**(`/api/sd/dashboard` 서버집계: 이번달 수주·출하대기·미청구·견적발송대기 + 파이프라인 + 최근수주 5).
>   - **권한 = 역할 가드 없음**(`ProtectedRoute` roles 미지정) — 백엔드도 `isAuthenticated()`. **누가 처리할지는 역할이 아니라 결재선이 정하고**, 서버가 `myTurn`으로 내려준다. 지금까지의 도메인(역할=권한)과 다른 축이라 학습 포인트.
>   - **⚠️ 백엔드 함정 2개(프론트에서 우회)**: ① **`ApprovalResponse.docLink`가 옛 Thymeleaf 경로**(`/fi/payments/1`, `/sd/quotations/1`)라 React 라우트(`/payments/1`)와 안 맞음 → 그대로 쓰면 링크 깨짐. `domain/status.js`의 `APPROVAL_DOC_ROUTE`로 docType→React 경로 **직접 매핑**(여신 상향처럼 화면 없는 문서는 링크 대신 "화면 없음"). ② **반려 사유가 `returnReason`에 안 담김** — 반송은 `returnReason`, **반려는 그 단계의 `steps[].comment`에만** 들어감 → `returnReason`만 보면 반려 문서 사유가 빈칸. `approvalReason(approval)` 헬퍼가 양쪽에서 꺼내도록 보정(상세·대시보드 상신현황 둘 다 적용).
>   - **🐞 같이 고친 것 — 착지 버그 완결**: FI 때 `HomeRedirect`로 고쳤던 걸 **대시보드가 역할 무관이라 더 단순하게** 정리(무조건 `/dashboard`). 추가로 **`LoginView`가 로그인 후 목적지를 `/quotations`로 하드코딩**하고 있어 같은 버그가 로그인 경로에 남아 있던 것 발견 → `/dashboard`로 수정.
>   - **✅ e2e 검증(실 API·AWS RDS, 결재자 `mgmt.dir@hyunwoo.com`=문재현 DIRECTOR/FINANCE)**: **⭐ FI 초안 → 승인 → 자동 전기 실동작**(전표 7914 DRAFT/postedAt=null → 승인 → **POSTED**/postedAt 채워짐) / 반송 → 결재 DRAFT + returnReason 기록, 원본 지급은 DRAFT 유지 / 재상신 → PENDING / **남이 회수 시도 → 409**("본인이 상신한 결재만") / 본인 회수 → WITHDRAWN / 반려 → REJECTED, 원본 지급 전기 안 됨(DRAFT 유지) / **내 차례 아닌 사람 승인 시도 → 409**("현재 결재/합의할 차례가 아닙니다") + `myTurn=false` / 알림 4종 수신(APPROVAL_REQUESTED·APPROVED·REJECTED·RETURNED) + 단건/전체 읽음 → 미확인 0 / 영업 대시보드 kim(SALES)=200 집계 정상, finance.mgr=**403**. `npm run build` 그린(193모듈, gzip 125KB), Vite 프록시 경유 `/dashboard`·`/approvals` 딥링크 200.
>   - **✅ 지난 세션 미검증분 2건 닫음**: ⓐ **발주 상신 → 결재 승인 → 발주 CONFIRMED 자동 전이** 실동작 확인(발주 id=3, 결재 id=14, 승인자 `purchase.mgr@hyunwoo.com`=황현우). ⓑ 그 CONFIRMED 발주로 **입고 전기 → 발주 RECEIVED 자동 전이**까지 확인(입고 id=972). 지난번 "결재 화면이 없어 CONFIRMED 발주를 못 만들어 e2e 못 태움"이 해소됨.
>   - **참고**: 입고 생성 DTO 라인 필드는 `unitPrice`가 아니라 **`unitCost`**(기존 `GoodsReceiptCreateView`는 이미 올바름 — curl 테스트 시 주의).
>
> - **✅ ERP React → TypeScript 전면 전환 완료(2026-07-17, hwlee님 결정)**: MES가 이미 TS로 전환돼 있어(작업 트리 미커밋) **양쪽 스택을 TS로 통일**. `.jsx`/`.js` **0개 남김**, ts/tsx 56개. 방식 = **전면 전환**(점진 X).
>   - **⭐ 결정적 근거 = 백엔드에 OpenAPI 스펙이 살아있음**: `http://localhost:8080/api-docs` (springdoc, **경로 124 / 스키마 136**). ⚠️ 경로가 `/v3/api-docs` 가 아니라 **`/api-docs`**(application.yml 에서 변경됨) — 404 나면 이것부터 확인. 타입을 손으로 쓰면 서버와 어긋나도 몰라서 '틀린 안정감'만 생기는데, 이 스펙이 있어 **정확도를 검증**할 수 있었다.
>   - **타입 전략 = MES와 동일(수기 `types/api.ts`, 생성 도구 X)**: Java record 와 1:1, 직렬화 규칙 주석(BigDecimal→number, LocalDate→string, enum→문자열 리터럴 유니온). **⚠️ springdoc 의 `required` 는 신뢰 불가** — Java record 필드를 전부 optional 로 내보낸다. 그래서 **필드명·타입만 스펙에서 가져오고 null 여부는 도메인 코드 기준**으로 판단했다.
>   - **✅ 타입 정확도 검증(수기 타입의 최대 리스크)**: 스크립트로 **타입 50개를 OpenAPI 스키마와 대조** → **"서버에 없는 필드 0"**(= 지어낸 필드 없음) 확인. 이 대조 스크립트는 스펙만 있으면 언제든 재실행 가능(DTO 바뀌면 다시 돌릴 것).
>   - **툴체인**: tsconfig 3종(app/node/references, MES와 동일 구조) + `strict`·`noUnusedLocals`·`noUnusedParameters`·`noFallthroughCasesInSwitch` + `verbatimModuleSyntax`(타입 import 런타임 소거 확인) + `jsx: react-jsx`. **빌드에 타입검사 포함** — `build: tsc -b && vite build`(MES는 `vue-tsc --build`), `type-check` 스크립트 별도. deps 추가: typescript ~5.9, @types/react, @types/react-dom, @types/node.
>   - **⭐ 타입으로 올린 도메인 규칙**(단순 전환을 넘어선 부분): ⓐ **`PaymentCreateRequest` 를 판별 유니온으로** — `RECEIPT`면 customerId 필수·vendorId 금지(`vendorId?: never`), `DISBURSEMENT`면 반대. **잘못된 조합이 컴파일에서 걸린다**(예전엔 400 응답으로만 알 수 있었음). ⓑ `StatusMap<K> = Record<K, StatusMeta>` — enum 값을 하나라도 빠뜨리면 컴파일 에러. ⓒ `StatusBadge<K>` 가 map·status 를 같은 K 로 묶어 **엉뚱한 상태맵에 엉뚱한 상태 전달 차단**. ⓓ 상태별 액션 함수 반환을 `string[]` → 액션 유니온으로. ⓔ 견적 `convert` 는 API 액션이 아니라 화면 이동이라 `QuotationButton = QuotationAction | 'convert'` 로 분리 — early return 이 있어야 `quotationAction()` 호출이 성립함을 타입이 증명.
>   - **🔍 TS가 잡아낸 것**: `ProductionOrderDetailView` 의 `avail?.lines || po.lines` — **두 응답의 라인 타입이 달라**(가용성 라인에만 onHandQty·sufficient) 한 배열처럼 다루고 있었음. **런타임은 `avail &&` 가드 덕에 실제로 안전했으나**, "materialLines 의 출처"와 "가드"가 보이지 않게 결합돼 깨지기 쉬운 구조 → `MaterialRow` 로 명시 정규화(재고 없는 경우 `null`).
>   - **⚠️ TS가 못 잡는 영역(중요)**: 상태 전이 규칙(POSTED만 취소)·차대 균형·myTurn 같은 **도메인 규칙은 타입 밖**이다. `domain/status.ts` 규칙이 서버와 어긋나도 컴파일은 통과하고 버튼만 잘못 보인다 → **서버 에러 메시지를 항상 화면에 표시**하는 게 여전히 필수. 파일 상단에 이 경고를 주석으로 박아둠.
...
>
> - **✅ ERP 데이터 시뮬레이터 추가(2026-07-21, hwlee님 요청 "안 쓰면 데이터가 없으니 매일 20~30개씩 자동 생성") = 데모/포트폴리오용 자동 데이터 생성기**: MES `ProductionSimulator` 와 같은 결로 **신규 `simulation/ErpDataSimulator.java`**. `@Scheduled(cron)` 로 매일 08~20시 2시간마다(7틱), 틱당 3~5건 무작위 생성 → 하루 약 20~35건. **실제 REST 와 동일한 서비스 메서드를 그대로 호출**해 상태전이·자동분개·재고반영·이벤트까지 정상 경로로 흐름. 전체 파이프라인 커버: 영업(견적·수주+확정·출하·청구)/구매(발주·무발주입고+전기)/생산(작업지시 초안 BOM전개)/회계(입출금 즉시전기)/인사(당일 근태). **각 활동 격리**(여신초과로 수주확정 실패, 재고부족으로 출하 실패, 근태 중복 등은 조용히 건너뜀 — 한 활동 실패가 틱 전체를 막지 않음). 마스터는 각 틱마다 `findAll` 로 로드(고객·거래처·완제품·창고·사원·거래처취급품목ACTIVE). **⚙️ 설정**: `erp.simulator.{enabled,cron,per-tick-min,per-tick-max}`(application.yml). **기본 비활성**, `application-prod.yml` 에서만 `enabled: ${ERP_SIMULATOR_ENABLED:true}` → **운영 서버(prod 프로파일)에서만 자동 가동**, 로컬/aws 는 꺼둠(개발·e2e 오염 방지). 서버 `.env` 에 `ERP_SIMULATOR_ENABLED=false` 로 토글 가능. 미인증 스케줄러 스레드라 `SecurityAuditorAware` 가 `createdBy="system"` 으로 fallback → 시뮬 생성분 식별 가능. **✅ `./gradlew compileJava` 그린**. ⚠️ **미커밋·미배포**(배포해야 실제 가동), ⚠️ MES 는 별도(ERP 작업지시 dispatch 안 함 → MES 신규 데이터는 자동생성 안 됨, 필요 시 추가 논의).
>
> - **✅ 시뮬레이터 결재 상신자를 팀장 → 부서 담당으로 교정(2026-07-21, hwlee님 관찰 "결재가 담당은 없고 팀장급에만 있는 것 같다")**: 맞는 지적이었음. 견적=`sales.mgr`·지급=`finance.mgr` 로 **팀장이 상신**하고 있었는데, `ApprovalLineResolver:64` 가 `if (email.equals(requester)) continue` 로 **상신자 자신을 결재선에서 건너뛰므로** 팀장 단계가 통째로 빠지고 본부장부터 시작 → 결재선이 1~2단계로 밋밋했다. **신규 계정 없이 기존 시드의 담당급 계정으로 교체**: 견적 = `kim@`(김영업, 국내영업1팀) / `sales.global@`(윤상현, 해외영업팀) **랜덤**, 지급 = `lee@`(이재무, 재무팀), 발주 = `purchase@`(박상현) 유지. 상신자는 클래스 상수(`SALES_REQUESTERS`/`PURCHASE_REQUESTER`/`PAYMENT_REQUESTER`)로 분리. 이제 **담당 → 팀장 → 본부장 → (고액) 대표** 로 결재선이 온전히 펼쳐진다. **커밋 `4592237` · main 푸시 · GitHub Actions 배포 성공 · `erp.hyunwoo.pro/actuator/health` UP 확인**. (⚠️ 위 항목의 "미커밋·미배포" 표기는 해소됨 — 시뮬레이터는 운영에서 가동 중.)
>   - **▶ 다음 할 일(2026-07-22 시작점) = 해외영업팀 부서장 공백 메우기**: `DEPT-SALESGL`(해외영업팀)은 V64 에서 `manager_id` 를 안 넣어 **팀장이 없다** → 윤상현 상신 건은 팀장을 건너뛰고 영업본부장으로 직행한다. 윤상현을 팀장으로 지정하면 본인이 상신자라 또 건너뛰므로, **해외영업 담당 직원+로그인 계정을 신설하는 마이그레이션(V73)** 을 만들고 윤상현을 해외영업팀장으로 앉히는 편이 낫다. (`V64__approval_org_setup.sql` 의 부서장 UPDATE + `V57__step4_accounts_login.sql` 의 계정/역할 INSERT 패턴 그대로. 역할=`SALES`, 비번 해시는 기존 pass1234 BCrypt 재사용.) 그 뒤 `SALES_REQUESTERS` 에 새 담당 계정을 추가하면 해외영업 라인도 담당→팀장→본부장이 된다.
>
...
>   - **✅ 포트폴리오 전략 수립 + 경력 섹션 전면 개편(2026-07-20)**: hwlee님이 목표를 명확히 밝힘 — **제조업(공장) IT 경력직 취업**. 목표가 정해지자 콘텐츠 우선순위가 바뀜(개인 프로젝트 중심 → 실무 경력과 양립). 신규 문서 2개: **`doc/포트폴리오-사이트-전략-및-구현계획.md`**(진단·결정·8단계 구현계획) + **`doc/포트폴리오-내용-작성-질문지.md`**(hwlee님 답변 완료).
>     - **⭐ 진단 = 본인이 꼽은 약점 3개 중 2개는 사실이 아님**: ⓐ "ERP 경력 없음" → **우정항공이 명백한 ERP 실무**(커스터마이징·쿼리최적화·대외연계). ⓑ "웹개발·유지보수라 경쟁력 없음" → 제조 사내 IT 업무의 실체가 레거시 유지보수라 **오히려 적합**. ⓒ 나이/경력직만 실재 허들 → **개인 프로젝트의 역할은 기술 자랑이 아니라 "새 도메인 학습 의지"의 증거**로 재포지셔닝.
>     - **⭐ 답변에서 발굴된 자산(기존 사이트에 한 글자도 없던 것)**: ①**ASP→전자정부프레임워크 전환 7개 사이트 단독 수행**(엑스아이티) — 제조 사내 IT에 가장 희소 ②**관세청 통관 EDI·KTNET/KCNET 연계**(우정항공) — 수출입 제조업 필수 ③**첼로스퀘어 = 데이터 제공자 역할**(우리 스케줄·운임을 삼성SDS 플랫폼 규격으로 노출) ④담당 규모 **약 30개 기관**(서울시청+23개 구청+지방시청) ⑤**임시테이블로 1분→10초** ⑥**서버 다운 원인 95% 해소** ⑦AWS **월 110만→60만원대**.
>     - **확정 방침**: 이력서 PDF **미게재**(유입=사람인/잡코리아 → **이미 이력서를 본 사람**이 옴 → 사이트는 요약본이 아니라 **한 줄이던 걸 증거와 함께 펼치는 곳**) / 전화번호 **미게재**(봇 수집→스팸) / 장비 연동 **주장 안 함**(C# 개발자 담당, MES 공백은 개인 프로젝트로만) / 타깃 세분화 안 함.
>     - **🔴 링크 판정(실측)**: `pudo24h.com`·`welltec.co.kr` **둘 다 HTTPS 미지원**(https 연결 실패) → hyunwoo.pro(HTTPS)에서 http 링크 시 브라우저 "안전하지 않음" 경고. **보안 취약점 개선 경력을 어필하는 페이지라 특히 부적절** → **링크 없이 "전체 개발" 서술만**. `ddakplay.com`만 링크(전체 유지보수·추가개발).
>     - **구현 완료 = S0·S1·S5**: ⓐ**S0** 🐞 푸터에 남아있던 시안 문구(`시안(mockup) — 콘텐츠는 자유롭게 교체하세요`) 제거 — 운영 사이트에 노출 중이었음. ⓑ**S1** 실무 경력 전면 재작성: 한 줄 요약 → **불릿 서술**(`.pts`), 우정항공에 `ERP`·엑스아이티에 `레거시 전환` 배지, 비공개 건은 `비공개 · 사내 시스템` 라벨. ⓒ**S5** 기술 스택에 `legacy/`·`cloud/`·`infra/` 카테고리 신설(전자정부·JSP·Spring XML·MyBatis·Oracle/MSSQL·프로시저·jQuery·Tomcat·Linux/Windows·셸·크론 + AWS/Oracle Cloud/KT Cloud) + 레거시 대응 가능함을 밝히는 주석 문단.
>     - **🐞 작업 중 잡은 사실오류**: 웰텍·위두쉽이 같은 회사(자회사 분사)라 하나로 병합했는데, **그 사이에 우정항공이 끼어 있어 연속 근무가 아님**(웰텍 2019.02–2022.05 → 우정항공 → 위두쉽 2023.08–2025.02). 병합 시 경력 왜곡이라 **분리 복원** + 위두쉽에 `웰텍이 분사한 자회사 · 재합류` 라벨(재합류는 오히려 긍정 신호).
>     - **✍️ 톤 수정(hwlee님 피드백 "과하고 오글거리고 AI스럽다")**: 초안이 설교조·과잉수식이었음 → 담백하게 재작성. 제거한 것 = 섹션 부제 "현업이 매일 쓰는 업무 시스템을 만들고 **지켜왔습니다**", 스택 주석 "오래된 코드 위에 서 있고 … **실제 업무의 큰 부분이라고 생각합니다**"(감상·설교), "해외 문의 **유입 경로를 만들었습니다**"(과장), 배지 `ERP 실무`→`ERP`. 구조도 **역할/연계/성과 dl 라벨 → 단순 불릿**으로(라벨링이 이력서 템플릿처럼 뻣뻣했음). **정렬도 관련도순 → 날짜 역순(desc)** 으로 환원(hwlee님 요청) = 딱플 → 위두쉽 → 우정항공 → 웰텍 → 엑스아이티 → 토모소프트 → 학력. 관련도순은 채용담당자가 시간순이 아닌 걸 혼란스러워할 위험도 있어 되돌리는 게 맞았음.
>     - **✅ 검증**: HTML 태그 균형 파서 통과(불일치 0) / **CDP `Emulation.setDeviceMetricsOverride`** 로 1280·768·375·320px 실측 — 전부 `scrollWidth ≤ viewport`(가로 넘침 0) / 데스크톱·모바일 스크린샷 육안 확인. ⚠️ macOS 창 최소폭 함정 때문에 `--window-size` 대신 CDP 사용(MES 때와 동일).
>     - **▶ 남은 단계**: **S2**(`왜 ERP·MES를 만들었나` 섹션 신설 — 재료 확보됨: "제조업으로 가고 싶은데 제대로 알고 싶었다" + "가장 어려웠던 건 정답지가 없는 것") · **S3**(Hero를 제조업 타깃으로) · **S4**(프로젝트 카드에 기술 하이라이트) · **S6**(`일하는 방식`) · **S7**(CSS 목업→실제 스크린샷) · S8(이력서 CTA는 미게재 확정으로 취소). **커밋 `6507ac6` · 배포 완료(2026-07-20)**.
>   - **✅ 방문 통계 추가·배포 완료(2026-07-20)** — `https://hyunwoo.pro/stats` (BasicAuth, ID `admin`). 커밋 `c14f2c2`·`e59d6f2`.
>     - **동기**: "정적 HTML인데 방문자 수를 볼 수 있나?" → **가능**. 기록 주체는 HTML이 아니라 **Caddy**이고, Caddy는 액세스 로그가 **기본 비활성**이라 그동안 아무것도 안 남고 있었다(켜기만 하면 됨).
>     - **구성**: Caddy 로그(JSON) → `jq` → Combined 형식 → **GoAccess** → 정적 HTML → `/stats/` 로 서빙(5분 주기). 신규 `hwlee-stats/`(alpine+goaccess+jq, 외부 포트 없음) + 볼륨 `caddy-logs`(caddy rw→stats ro)·`stats-html`(stats rw→caddy ro).
>     - **⭐ 핵심 함정 = Cloudflare 뒤의 방문자 IP**: 전 트래픽이 CF를 거쳐 `remote_ip`가 **전부 CF 서버 IP** → 그대로 집계하면 방문자 수가 무의미. 전역 `trusted_proxies`(CF 대역 한정)+`client_ip_headers Cf-Connecting-Ip` 로 `client_ip` 복원. **실측 확인**: `client_ip 121.184.171.205` / `remote_ip 172.64.217.9`(CF)로 분리 기록됨.
>     - **⭐ 안전장치**: `STATS_PASSWORD_HASH` 기본값을 **폐기한 랜덤 문자열의 유효한 bcrypt 해시**로 박음. 빈 문자열이면 `basic_auth`가 죽으며 **Caddy 전체 기동 실패 → erp·mes·포트폴리오 동반 다운**(재현 확인). 이 기본값 덕에 `.env` 미설정 시 `/stats`만 잠기고 사이트는 정상.
>     - **🐞 배포 중 잡은 버그 2 + 실배포 1**: ⓐGoAccess는 **출력 형식을 확장자로 판단** → 임시파일 `.tmp`면 즉사(→`index.tmp.html`). ⓑ`log_skip /stats/*`가 슬래시 없는 `/stats`(302)를 못 걸러 **내 조회가 방문자로 잡힘**(→`/stats*`). ⓒ**실배포 401**: docker compose가 `.env` 값의 `$`를 변수로 해석해 bcrypt 해시가 **60자→50자로 잘림**(`printenv`로 확인) → `.env`에는 **`$`를 `$$`로** 넣어야 함(문서 수정 `e59d6f2`).
>     - **✅ 검증(운영 실측)**: hyunwoo.pro·erp·mes·www **전부 200** / `/stats` 비인증·오인증 **401**, 정인증 **200**(708KB 리포트) / 컨테이너 11개 Up / 로그 수집·집계 정상.
>     - **⚠️ 한계**: 과거 데이터 없음(**켠 시점부터** 누적. 그 이전은 Cloudflare 대시보드에만) · **국가별 통계 없음**(GeoIP mmdb 미포함 — 필요시 `--geoip-database` 추가) · 롤링되어 `.gz`가 된 과거 로그는 집계 제외 · 서버 `.env` 백업 2개 남김(`.env.bak.20260720-115231`, `.env.bak.pre-escape`).
...
> - **⚠️ 로그인 API 주의**: 요청 필드가 `email` 이 아니라 **`username`**(값은 이메일). curl e2e 할 때 자주 헷갈림.
> - **⚠️ 커밋 상태**: 생산 PP까지는 커밋·푸시됨(`a6aa6f6`). **재무(FI) + 전자결재/대시보드 + TypeScript 전면 전환 분량은 미커밋**(ERP 프론트 65파일 변경) — 다른 PC에서 이어가려면 커밋·푸시 필요. **MES TS 전환분도 미커밋 상태로 작업 트리에 있음**(hwlee님 작업분, 이번 세션에서 건드리지 않음).
> - **⚙️ TS 명령어**: `npm run type-check`(tsc -b) / `npm run build`(타입검사+빌드) / `npm run build-only`(빌드만). **새 도메인 추가 시 순서** = `types/api.ts` 에 DTO 타입 먼저 → `api/*.ts` → 화면. DTO 가 바뀌면 `types/api.ts` 부터 고친다.
> - **⚠️ RDS에 남은 e2e 데이터(전자결재분 추가)**: 결재 APV-20260717-001(승인됨)·002(회수됨)·나머지 반려/대기 + 그에 딸린 전표·지급(`[e2e]` 표시) / 발주 id=3(RECEIVED, `[e2e] 결재 연동 테스트`)·입고 id=972(전기됨, **재고·재고이동에 실제 반영됨**). 데모용으로 두거나 DB에서 직접 정리.
> - **⚙️ ERP 프론트 실행법**: 백엔드 `:8080` 기동 상태에서 `cd hwlee-erp/frontend && npm install && npm run dev` → http://localhost:5174.
>
> #### ▶▶ 다음 세션 시작점 (2026-07-16 저장)
> **지금까지**: MES Vue SPA = 화면 4종(작업지시 목록·접수폼·현장실행 상세·설비 가동현황) + 품질검사 패널 + **백엔드 생산 시뮬레이터**(가동 켜면 자동 생산) + 실시간 폴링(생산량·가동률) 완성·검증. **+ 정적 배포 구성 완료(mes.hyunwoo.pro 서빙, 위 항목)**. MES Vue 1차 트랙 기능·배포 모두 일단락. **+ ERP React SPA(견적·수주) 착수 1차 완료**(위 블록).
> - **⚠️ 먼저 할 일 = 커밋·푸시** → push하면 GitHub Actions 자동배포로 `mes.hyunwoo.pro`가 Vue SPA로 전환됨. 배포 후 브라우저 육안 확인(딥링크·`/api` 실데이터) 권장.
> - **다음 진도 후보(택1)**:
>   1. ~~MES 정적 빌드 배포~~ ✅ **완료(2026-07-16, 위)** — 커밋·푸시 후 브라우저 육안 확인만 남음.
