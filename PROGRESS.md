...
>     - **구현 완료 = S0·S1·S5**: ⓐ**S0** 🐞 푸터에 남아있던 시안 문구(`시안(mockup) — 콘텐츠는 자유롭게 교체하세요`) 제거 — 운영 사이트에 노출 중이었음. ⓑ**S1** 실무 경력 전면 재작성: 한 줄 요약 → **역할/연계/성과 구조**(`.facts` dl), 우정항공(ERP 실무 배지)·엑스아이티(레거시 전환 배지)·딱플을 **관련도 높은 순**으로 재배치(섹션 부제에 명시), 비공개 건은 `비공개 · 사내 시스템` 라벨. ⓒ**S5** 기술 스택에 `legacy/`·`cloud/`·`infra/` 카테고리 신설(전자정부·JSP·Spring XML·MyBatis·Oracle/MSSQL·프로시저·jQuery·Tomcat·Linux/Windows·셸·크론 + AWS/Oracle Cloud/KT Cloud) + 레거시 대응 가능함을 밝히는 주석 문단.
