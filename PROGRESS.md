...
>     - **▶ 남은 단계**: **S2**(`왜 ERP·MES를 만들었나` 섹션 신설 — 재료 확보됨: "제조업으로 가고 싶은데 제대로 알고 싶었다" + "가장 어려웠던 건 정답지가 없는 것") · **S3**(Hero를 제조업 타깃으로) · **S4**(프로젝트 카드에 기술 하이라이트) · **S6**(`일하는 방식`) · **S7**(CSS 목업→실제 스크린샷) · S8(이력서 CTA는 미게재 확정으로 취소). **미커밋·미배포**.
