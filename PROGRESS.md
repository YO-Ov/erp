...
>
> #### 📊 React 전환 도메인 현황 (2026-07-16 기준)
> - ✅ **인증**(로그인·JWT Bearer·역할가드) / ✅ **영업 SD**(견적·수주·출하·청구) / ✅ **구매 MM**(발주·입고) / ✅ **생산 PP**(작업지시)
> - ⬜ 재무 FI · 인사 HR · 재고조회 · BOM/MRP · 전자결재 · 알림 · 대시보드 · 마스터관리
...
>
> #### ▶▶ 다음 세션 시작점 — ERP React 전환 이어가기 (2026-07-16 퇴근 저장)
> **지금까지**: ERP React SPA = 인증 + **영업(SD 견적·수주·출하·청구) + 구매(MM 발주·입고) + 생산(PP 작업지시)** 3대 업무 흐름 완성·실 API e2e 검증 끝. 위 "React 전환 도메인 현황"이 전체 지도.
> - **▶ 다음 진도(택1, 권장 순서)**:
>   1. **재무(FI)** — 전표(journal-entries)·지급(payments)·계정(accounts). 지원부서 축. 권한 FINANCE/ADMIN(리포트는 DIRECTOR도). 전표는 차대변 균형 검증(422 UNBALANCED_JOURNAL) UX가 포인트.
>   2. **재고 조회**(현재고 stocks·이동이력 stock-movements) — MM 조회 완결, 화면 단순(읽기전용)이라 빠름. 지금까지 만든 출고/입고/생산이 재고에 어떻게 반영됐는지 보이는 화면.
>   3. **인사(HR)** — 사원·근태·급여. 권한 HR/ADMIN.
>   4. **전자결재/대시보드** — 발주·견적 상신이 실제 결재로 흐르는 걸 화면화(지금은 상신만 되고 승인 화면이 React엔 없음). CONFIRMED 발주 e2e를 못 태운 것도 이게 없어서였음.
> - **재개 방법**: ⓐ 백엔드 `:8080` 기동 확인(현재 떠 있음) ⓑ `cd hwlee-erp/frontend && npm run dev` → http://localhost:5174 ⓒ 로그인 빠른선택에서 역할 골라 확인. **새 도메인은 정립된 공통 패턴(위) 그대로** 복제 = api/ + domain status + 목록/상세/작성 + App 라우트·네비·권한.
> - **⚠️ 커밋 상태**: 이 시점(생산 PP까지)을 커밋·푸시함. `hwlee-erp/frontend/` 전체가 git에 포함(node_modules·dist는 자체 .gitignore 제외).
