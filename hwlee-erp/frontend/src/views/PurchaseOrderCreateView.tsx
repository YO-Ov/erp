...
import type { PurchaseOrder, PurchaseOrderCreateRequest } from '../types/api'
...
export default function PurchaseOrderCreateView() {
  ...
  const { data: vendorItems = [] } = useQuery({
    ...
    queryFn: () => listVendorItems(Number(vendorId)),
    enabled: !!vendorId,
  })
  ...
  const mutation = useMutation<PurchaseOrder, Error, PurchaseOrderCreateRequest>({
    mutationFn: (body) =>
  ...
  function onVendorChange(newVendorId: string) {
