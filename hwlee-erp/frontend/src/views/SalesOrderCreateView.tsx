...
/** 작성이냐 수정이냐에 따라 DTO 가 다르다 — 수정 DTO 엔 customerId·quotationId 가 없다(변경 불가). */
type SalesOrderFormBody = SalesOrderCreateRequest | SalesOrderUpdateRequest
...
const emptyLine = (): LineInput => ({ itemId: '', orderQty: 1, unitPrice: 0 })
...
export default function SalesOrderCreateView() {
  ...
  const [salespersonId, setSalespersonId] = useState<number | null>(null) // 폼엔 없지만 수정 시 보존
  ...
  const mutation = useMutation<SalesOrder, Error, SalesOrderFormBody>({
    // customerId 유무가 곧 작성/수정 구분이다 — 수정 DTO 엔 그 필드가 없다.
    mutationFn: (body) =>
      'customerId' in body ? createSalesOrder(body) : updateSalesOrder(id, body),
